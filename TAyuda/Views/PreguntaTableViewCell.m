...
- (void)setUp{
    ...
    respuestaView.font = [UIFont fontWithName:@"FrutigerLTStd-Light" size:[respuestaView.font pointSize]];
    ...
    respuestaView.textContainerInset = UIEdgeInsetsMake(6, HORIZONTAL_PADDING, 6, HORIZONTAL_PADDING * 2);
    ...
    [tramiteButton setTitle:@"Trámite asociaso" forState:UIControlStateNormal];
...
- (void)drawRect:(CGRect)rect{
    self.respuestaView.frame = CGRectMake(15, 0, rect.size.width - 20, rect.size.height);
    ...
    if(self.showButtons){
        ...
        tramiteButtonRect.origin = CGPointMake(rect.size.width - tramiteButtonRect.size.width - (2 * HORIZONTAL_PADDING), (EXTRA_INSET - tramiteButtonRect.size.height) / 2.0);
...
+ (CGFloat)heightOfText:(NSString *)text withWidth:(CGFloat)width{
    ...
    font = [UIFont fontWithName:@"FrutigerLTStd-Light" size:[font pointSize]];
...
+ (CGFloat)heightOfPregunta:(Pregunta *)pregunta withWidth:(CGFloat)width{
    ...
    height = height + 30 + ((pregunta.enlaceExterno  && [pregunta.enlaceExterno length] > 0) || pregunta.tramite? EXTRA_INSET: 0);
